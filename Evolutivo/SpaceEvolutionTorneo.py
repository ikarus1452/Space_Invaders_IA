...
# Elige nuevos padres al elegir los mejores entre pequeños grupos
def select_parents_tournament(population, scores, num_parents, tournament_size=5):
    parents = []
    population_size = len(population)
    
    for _ in range(num_parents):
        # Escoge un grupo al azar
        tournament_indices = np.random.choice(population_size, tournament_size, replace=False)
        # Obtiene sus puntajes
        tournament_scores = [scores[i] for i in tournament_indices]
        # Compara el grupo y guarda el mejor
        winner_idx = tournament_indices[np.argmax(tournament_scores)]
        # Propaga y repite hasta acabar
...
initial_mutation_power = 0.9  # Tasa de mutación más alta al principio
final_mutation_power = 0.001   # Tasa de mutación más baja hacia el final
...
num_generations = 50 #Cantidad de veces que se entrenará la población
...
for generation in range(num_generations):
    ...
    parents = select_parents_tournament(population, scores, num_parents)
